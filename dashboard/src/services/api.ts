...
class ApiService {
  ...
  private handleError(error: unknown): ApiError {
    if (axios.isAxiosError(error)) {
      ...
      const data = axiosError.response.data as { message?: string; error?: string; errors?: Array<{ msg?: string; path?: string; value?: unknown }> }
        ...
        case 400:
          ...
          if (data.errors && Array.isArray(data.errors)) {
            const validationMessages = data.errors
              .map(err => `${err.path}: ${err.msg || 'Validation error'}`)
              .join(', ')
            return {
  ...
  async login(secretKey: string, serviceKey: string) {
    try {
      ...
      const response = await this.api.post('/login', { secretKey, serviceKey })
      ...
      if (token) {
        ...
        throw {
          message: 'No token received from server. Response: ' + JSON.stringify(response.data),
          code: 'NO_TOKEN',
        }
      }
  ...
  async updateConfiguration(config: unknown) {
    try {
      ...
      const response = await this.api.put('/private/configuration', config)
  ...
  async reloadConfiguration() {
    try {
      const response = await this.api.post('/private/configuration/reload')
      return response.data.content || response.data
    } catch (error) {
      throw error
    }
