 ...
 * @return string
 */
export const selfDescriptionProcessor = (
    target: string,
    ...
    if (contractURL.includes('bilaterals')) {
        ...
        const targets = contract?.policy
            .map((p: { permission: any }) =>
                p.permission.map((per: { target: any }) => per.target)
            )
            .flat();
    ...
    // Ecosystem case
        ...
        const targets = contract?.serviceOfferings
            .map((p: { policies: any }) =>
                p.policies
                    .map((policy: { permission: any }) =>
                        policy.permission.map(
                            (per: { target: any }) => per.target
                        )
                    )
                    .flat()
            )
...
const assignRightTarget = (targets: string[], target: string) => {
    ...
    if (targets && targets.length > 0 && targets.includes(target)) {
        ...
        if (targets.includes(pathElements[pathElements.length - 1]))
            returnTarget = splitTarget;
    } else {
        returnTarget = target;
    }
    return returnTarget;
