...
export const consentServiceGiveConsent = async (req: Request) => {
    try {
        ...
        const response = await axios.post(
            await verifyConsentUri(
                `consents${
                    triggerDataExchange ? '?triggerDataExchange=true' : ''
                }`
            ),
            { ...req.body },
            {
...
export const consentServiceResume = async (
    ...
    try {
        ...
        if (response.status === 200 && response?.data?.consumerUserIdentifier) {
...
const populate = async (response: any) => {
    ...
    const [...purposeResponses] = await Promise.all([
        ...response.data.purposes.map((purpose: any) =>
            axios.get(purpose?.purpose)
        ),
    ]);
    ...
    await Promise.all(
        dataResponsesMap.map(async (data: any) => {
            const [...dataResourceResponses] = await Promise.all([
                ...data.dataResources.map((resource: string) =>
                    axios.get(resource)
                ),
            ]);
    ...
    await Promise.all(
        purposeResponsesMap.map(async (data: any) => {
            const [...softwareResourceResponses] = await Promise.all([
                ...data.softwareResources.map((resource: string) =>
                    axios.get(resource)
                ),
            ]);
            data.softwareResources = softwareResourceResponses?.map(
                (dt) => dt?.data
            );
            return data;
        })
