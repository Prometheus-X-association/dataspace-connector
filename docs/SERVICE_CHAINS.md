...
> This payload can be configurable through the `serviceChainParams` field in the payload when triggering an exchange, refer to the [Query Parameters documentation](./QUERY_PARAMS.md#applying-query-parameters-to-a-specific-resource) for more information.
> 
> By using the flag `skipBodyProcessing` set to `true`, the PDC will forward the data as-is without any alteration.
```json
{
